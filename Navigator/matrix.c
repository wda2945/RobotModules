...
Matrix alloc_matrix(int rows, int cols) {int i,j;
  Matrix m;
  m.rows = rows;
  m.cols = cols;
...
void free_matrix(Matrix m) {int i;
  assert(m.data != NULL);
  for (i = 0; i < m.rows; ++i) {
    free(m.data[i]);
  }
...
void set_matrix(Matrix m, ...) {int i,j;
  va_list ap;
  va_start(ap, m);
...
void set_identity_matrix(Matrix m) {int i,j;
...
void copy_matrix(Matrix source, Matrix destination) {int i,j;
...
void print_matrix(Matrix m) {int i,j;
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) {
      if (j > 0) {
	printf(" ");
...
void add_matrix(Matrix a, Matrix b, Matrix c) {int i,j;
...
void subtract_matrix(Matrix a, Matrix b, Matrix c) {int i,j;
  assert(a.rows == b.rows);
  assert(a.rows == c.rows);
  assert(a.cols == b.cols);
  assert(a.cols == c.cols);
...
void subtract_from_identity_matrix(Matrix a) {int i,j;
...
void multiply_matrix(Matrix a, Matrix b, Matrix c) {int i,j,k;
...
void multiply_by_transpose_matrix(Matrix a, Matrix b, Matrix c) {int i,j,k;
  assert(a.cols == b.cols);
  assert(a.rows == c.rows);
...
void transpose_matrix(Matrix input, Matrix output) {int i,j;
...
int equal_matrix(Matrix a, Matrix b, double tolerance) {int i,j;
  assert(a.rows == b.rows);
  assert(a.cols == b.cols);
  for (i = 0; i < a.rows; ++i) {
    for (j = 0; j < a.cols; ++j) {
...
void scale_matrix(Matrix m, double scalar) {int i,j;
  assert(scalar != 0.0);
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) {
      m.data[i][j] *= scalar;
...
void scale_row(Matrix m, int r, double scalar) {int i;
  assert(scalar != 0.0);
  for (i = 0; i < m.cols; ++i) {
    m.data[r][i] *= scalar;
  }
...
void shear_row(Matrix m, int r1, int r2, double scalar) {int i;
  assert(r1 != r2);
  for (i = 0; i < m.cols; ++i) {
...
int destructive_invert_matrix(Matrix input, Matrix output) {int i,j;
